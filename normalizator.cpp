...
cv::Mat Normalizator::normalize(FaceData data)
{
	...
	double angle = atan2((double) (data.leye.y - data.reye.y), (double) (data.leye.x - data.reye.x)) - CV_PI; //rotation angle in RADIANS
	//rotation
	cv::Mat trans = cv::getRotationMatrix2D(data.faceCenter, (double) (180 / CV_PI) * angle, 1.0); //conversion to degrees needed
	cv::warpAffine(face, face, trans, cv::Size(face.cols, face.rows));
	//cv::imwrite("rotated.jpg", face);
	...
	//std::cout << "original eyes pos: (" << data.leye.x << ", " << data.leye.y << "); (" << data.reye.x << ", " << data.reye.y << ")\n";
	...
	data.leye.x += data.faceCenter.x; data.leye.y += data.faceCenter.y;
	data.reye.x += data.faceCenter.x; data.reye.y += data.faceCenter.y;
	//std::cout << "after rotation by " << (180 / CV_PI) * angle << ": (" << data.leye.x << ", " << data.leye.y << "); (" << data.reye.x << ", " << data.reye.y << ")\n";
